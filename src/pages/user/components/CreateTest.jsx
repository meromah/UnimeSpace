...
import CreateTestFromJson from "./CreateTestFromJson";
...
const CreateTest = ({ onCancel = undefined }) => {
  ...
  const [showCreateFromJson, setShowCreateFromJson] = useState(false);
  ...
  const onCreateFromJson = (e) => {
    e.preventDefault();
    setShowCreateFromJson(true);
  };
    ...
    >
      <div
        className={`flex flex-col justify-between bg-white dark:bg-neutral-900 h-full gap-4 p-6 ${
          isPopUp
            ? `w-full overflow-y-auto rounded-lg border border-neutral-200 dark:border-neutral-700 md:m-6 ${showCreateFromJson ? "md:max-w-5xl" : "md:max-w-3/4"}`
            : "w-full"
        }`}
      >
        ...
        {!isDraftsOpen && (
          <>
              <div className="flex items-center justify-between mb-2 select-none">
                ...
                <div className="m-5 flex gap-4">
                    ...
                    {showCreateFromJson ? (
                      <button
                        type="button"
                        onClick={() => setShowCreateFromJson(false)}
                        className="px-4 py-2 text-sm font-medium text-white bg-primary-blue hover:bg-primary-blue/90 dark:bg-primary-blue dark:hover:bg-primary-blue/90 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-blue/50 transition-colors cursor-pointer shadow-sm"
                      >
            ...
            {showCreateFromJson ? (
              <CreateTestFromJson
                onCancel={() => setShowCreateFromJson(false)}
              />
            ) : (
              <>
                <div
